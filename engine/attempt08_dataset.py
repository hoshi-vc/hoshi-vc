...
class Dataset08(Dataset):
  def __init__(
    ...
    for d, dref, faiss, speaker_id in zip(dirs, dirs_ref, indices, speaker_ids):
      ...
      self.max_ref_lens[d] = np.load(dref / "melspec.npy", mmap_mode="r").shape[0]
  ...
  def load_entry(self, d: str, speaker_id: int, start: int, frames: int) -> Feats08:
    end = start + frames
