...
# Based on attempt08b, attempt08 (wandb:attempt08/qmnaevyy), attempt06aa (wandb:attempt06/gzdtw6p4)

# これまでの試行のうちうまく行ったらしいものをまとめてみる
# 08: より良いサンプリング
# 06aa: speaker_d, club_ksp (spd_rot 系は取り込めなかった)

# TODO: バグ修正前の実験のやり直しなどをする・結果をちゃんと整理する
...
from engine.lib.utils import AttrDict, hide_warns
from engine.prepare import Preparation
from engine.utils import (DATA_DIR, BaseLightningModule, LinearSchedule, fm_loss, log_attentions, log_audios2, log_spectrograms, log_spksim1,
                          new_checkpoint_callback_wandb, new_wandb_logger, setup_train_environment, step_optimizer)
...
class VCModule(BaseLightningModule):
  ...
  def _process_batch(
    ...
    opt_model, opt_club, opt_spd = self.optimizers()
  ...
  def training_step(self, batch: Entry08, batch_idx: int):
    ...
    opt_model, opt_club, opt_spd = self.optimizers()
    sch_model, sch_club, sch_spd = self.lr_schedulers()
  ...
  def configure_optimizers(self):
    ...
    return [opt_model, opt_club, opt_spd], [sch_model, sch_club, sch_spd]
