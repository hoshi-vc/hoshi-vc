...
class VCModule(BaseLightningModule):
  ...
  def validation_step(self, batch: Entry08, batch_idx: int):
    ...
    if batch_idx == 0:
      mel_v, y_v, ymel_v, (attn1_v, attn2_v), _ = self._process_batch(batch, step=step, e2e=True, e2e_frames=None)
      names = [f"{i:02d}" for i in range(len(mel))]
      log_attentions(self, names, attn1_v, "Attention 1")
      log_attentions(self, names, attn2_v, "Attention 2")
...
if __name__ == "__main__":
  ...
  datamodule = DataModule08(P, frames=256, frames_ref=64, n_refs=32, ref_max_kth=64, batch_size=8, n_batches=2000, n_batches_val=200, num_workers=12)
