...
class VCModel(nn.Module):
  def __init__(self, hdim: int):
    ...
    self.phoneme_embed = nn.Embedding(400, w2v2_dim)
    ...
    self.lookup = MultiHeadAttention(kv_dim, kv_dim, 16, dropout=0.2, hard=True)
  ...
  def forward(self, batch: Entry08):
    ...
    ref_phoneme_i = torch.stack([o.phoneme_i for o in batch.ref]).flatten(0, 1)
    ref_phoneme_v = torch.stack([o.phoneme_v for o in batch.ref]).flatten(0, 1)
    ref_mel = torch.stack([o.mel for o in batch.ref]).flatten(0, 1)

    src_energy = batch.src.energy
    src_pitch_i = batch.src.pitch_i
    src_phoneme_i = batch.src.phoneme_i
    src_phoneme_v = batch.src.phoneme_v
    src_mel = batch.src.mel

    # (...) -> (n_refs*n_batch, seq_len, dim)
    ref_energy = self.forward_energy(ref_energy)
    ref_pitch = self.forward_pitch(ref_pitch_i)
    ref_w2v2 = self.forward_phoneme(ref_phoneme_i, ref_phoneme_v)
    ...
    src_w2v2 = self.forward_phoneme(src_phoneme_i, src_phoneme_v)
...
if __name__ == "__main__":
  ...
  datamodule = DataModule08(P, frames=256, frames_ref=64, n_refs=32, ref_max_kth=64, batch_size=8, n_batches=2000, n_batches_val=200)
