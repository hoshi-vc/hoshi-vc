...
from engine.utils import (DATA_DIR, BaseLightningModule, fm_loss,
                          log_attentions, log_audios2, log_spectrograms,
                          log_spksim, new_checkpoint_callback_wandb,
                          new_wandb_logger, rotate_dim0,
                          setup_train_environment, shuffle_dim0,
                          step_optimizer)
...
class VCModule(BaseLightningModule):
  def __init__(self,
    ...
    self.club_ksp = CLUBSampleForCategorical3(
        ...
        logvar=nn.Sequential(
            ...
            nn.ReLU(),
            Transpose(1, 2),
            weight_norm(nn.Conv1d(ksp_hdim, ksp_hdim, kernel_size=3, padding=1)),
            nn.ReLU(),
            weight_norm(nn.Conv1d(ksp_hdim, ksp_hdim, kernel_size=5, padding=2, stride=2)),
            nn.ReLU(),
            weight_norm(nn.Conv1d(ksp_hdim, ksp_hdim, kernel_size=5, padding=2, stride=2)),
            nn.ReLU(),
            weight_norm(nn.Conv1d(ksp_hdim, ksp_hdim, kernel_size=3, padding=1)),
  ...
  def _process_batch(self,
    ...
    club_sp_y = batch[0].speaker.repeat(1, ref_key.shape[1] // 4)
    club_sp_n = batch[0].speaker.reshape(-1)
    ...
    if train:
      step_optimizer(self, opt_club, total_club, self.grad_clip)
