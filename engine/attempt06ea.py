...
from engine.utils import (DATA_DIR, BaseLightningModule, log_attentions, log_audios2, log_spectrograms, log_spksim, new_checkpoint_callback_wandb,
...
class VCModel(nn.Module):
  ...
  def forward(self, batch: IntraDomainEntry4, src_ref_start: int, src_ref_len: int, n_rotated=0):
    ...
    tgt_value1, attn = self.lookup(src_key, ref_key, ref_value, need_weights=True)
    o = self.joint(torch.cat([tgt_value1, src_energy, src_pitch], dim=-1))
    tgt_value2, attn = self.lookup2(o, ref_key, ref_value2, need_weights=True)
    tgt_value = tgt_value1 + tgt_value2
    ...
    for i in range(n_rotated):
      ...
      tgt_value1_rotated, _ = self.lookup(src_key_rotated, ref_key, ref_value, need_weights=False)
      o = self.joint(torch.cat([tgt_value1_rotated, src_energy_rotated, src_pitch_rotated], dim=-1))
      tgt_value2_rotated, _ = self.lookup2(o, ref_key, ref_value2, need_weights=False)
      tgt_value_rotated = tgt_value1_rotated + tgt_value2_rotated
...
if __name__ == "__main__":
  ...
  assert PROJECT.startswith("attempt06")
  PROJECT = "attempt06"
